...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        p = self.place
        distance = 0
        while not p.is_hive and distance <= self.upper_bound:
            if (p.bees and self.lower_bound <= distance <= self.upper_bound):
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        if (other and not self.ant_contained and not other.is_container):
            return True
        return False
        # END Problem 8a
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        # BEGIN Problem 10
        "*** YOUR CODE HERE ***"
        assert insect is not None, "insect should not be None"
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def construct(cls, gamestate):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        if (not cls.has_queen):
            queen = super().construct(gamestate)
            if (queen):
                cls.has_queen = True
                return queen
    ...
    def action(self, gamestate):
        ...
        while p:
            if (p.ant):
                ...
                if (ant.is_container and ant.ant_contained and ant.ant_contained not in self.doubled):
