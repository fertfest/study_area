def make_bank(balance):
    ...
    def deposit(amount):
        ...
        balance += amount
        return balance
    methods = {"withdraw": withdraw, "deposit": deposit}

    def bank(message, amount):
        "*** YOUR CODE HERE ***"
        method = methods.get(message)
        if (method):
            return methods[message](amount)
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, password_input):
        nonlocal incorrect
        if (len(incorrect) == 3):
            return "Too many incorrect attempts. Attempts: " + str(incorrect)
        if (password_input == password):
            return correct_password(amount)
        incorrect.append(password_input)
...
def merge(incr_a, incr_b):
    ...
    while (next_a is not None and next_b is not None):
        if (next_a < next_b):
            ...
            dup_value = next_a
            yield next_a
            while (next_a == dup_value):
                next_a = next(iter_a, None)
            while (next_b == dup_value):
                next_b = next(iter_b, None)
