...
def about(topic):
    ...
    def select(paragraph):
        word_list = split(lower(remove_punctuation(paragraph)))
...
def shifty_shifts(start, goal, limit):
    ...
    """
    # BEGIN PROBLEM 6
    def shifty_shifts_helper(start, goal, limit, curdiff, index):
        if (curdiff > limit):
            return curdiff
        if (index >= len(start) or index >= len(goal)):
            curdiff = curdiff + abs(len(start) - len(goal))
            return curdiff
        if (start[index] == goal[index]):
            return shifty_shifts_helper(start, goal, limit, curdiff, index + 1)
        return shifty_shifts_helper(start, goal, limit, curdiff + 1, index + 1) 
    return shifty_shifts_helper(start, goal, limit, 0, 0)
    # END PROBLEM 6
...
def meowstake_matches(start, goal, limit):
    ...
    if len(start) == 0 or len(goal) == 0: # Fill in the condition
        ...
        # BEGIN
        "*** YOUR CODE HERE ***"
        print("DEBUG: ", min(add_diff, remove_diff, substitute_diff))
...
def time_per_word(times_per_player, words):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    num_of_players = len(times_per_player)
    num_of_words = len(words)
    timestamps = [[0] * num_of_words for times in times_per_player]

    i = 0
    while (i < num_of_players):
        j = 0
        while j < num_of_words:
            timestamps[i][j] = times_per_player[i][j+1] - times_per_player[i][j]
            print("DEBUG: ", i, j)
            j += 1
        i += 1
...
def fastest_words(game):
    ...
    result = [[] for _ in range(num_of_players)]
    i = 0
    while (i < len(all_words(game))):
        j = 0
        fastest_time = 99999
        fastest_index_of_players = 0
        while j < num_of_players:
            if (time(game, j, i) < fastest_time):
                fastest_time = time(game, j, i)
                fastest_index_of_players = j
            j += 1
        result[fastest_index_of_players] += [word_at(game, i)]
        i += 1
    return result
    # END PROBLEM 10
