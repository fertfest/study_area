...
def reverse_recursive(lst):
    ...
    if (len(lst) <= 1):
        return lst
    with_out_last = reverse_recursive(lst[0:-1])
    with_out_last.insert(0, lst[-1])
    return with_out_last
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    blank_city = make_city('', lat, lon)
    a_from_destination = distance(blank_city, city_a)
    b_from_destination = distance(blank_city, city_b)
    if (a_from_destination < b_from_destination):
...
def add_chars(w1, w2):
    ...
    "*** YOUR CODE HERE ***"
    result = ''
    def add_chars_helper(w1, w2, i1, i2):
        nonlocal result
        if (i1 == len(w1)):
            result += w2[i2:]
            return
        ...
        if (w1[i1] != w2[i2]):
            result += w2[i2]
            add_chars_helper(w1, w2, i1, i2 + 1)
        else:
            add_chars_helper(w1, w2, i1 + 1, i2 + 1)
    add_chars_helper(w1, w2, 0, 0)
    return result
